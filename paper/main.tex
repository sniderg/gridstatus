...
We compare our LightGBM model against naive baselines using the Mean Absolute Scaled Error (MASE), a scale-independent metric proposed by \citet{hyndman2006another}:
\begin{equation}
    \text{MASE} = \frac{\text{MAE}_{\text{forecast}}}{\text{MAE}_{\text{naive}}} = \frac{\frac{1}{H}\sum_{t=1}^{H}|y_t - \hat{y}_t|}{\frac{1}{T-m}\sum_{t=m+1}^{T}|y_t - y_{t-m}|}
\end{equation}
where $H$ is the forecast horizon, $T$ is the training set size, and $m=24$ is the seasonal period (hourly day-ahead). MASE $< 1$ indicates the model outperforms the seasonal naive baseline; MASE $= 0.5$ means errors are 50\% smaller than naive. Table~\ref{tab:benchmark} and Figure~\ref{fig:benchmark} summarize 2025 test set performance.
...
\begin{table}[H]
    ...
    \begin{tabular}{lcc}
        ...
        Model & MAE & MASE \\
        \midrule
        \textbf{LightGBM} & \textbf{\$9.46} & \textbf{0.813} \\
        Naive (24h) & \$11.61 & 1.000 \\
        Seasonal (168h) & \$14.16 & 1.218 \\
        \bottomrule
    \end{tabular}
\end{table}
...
\begin{figure}[H]
    ...
    \caption{Model performance during three extreme weather events. The top panel shows Winter Storm Uri, where the model failed to capture the \$9,000/MWh scarcity pricing. The middle and bottom panels show performance during a summer heatwave and a strictly cold event, respectively.}
    \label{fig:extreme}
\end{figure}
...
The achieved average MAE of \$9.35/MWh compares favorably to benchmark studies on ERCOT price forecasting. For context:
\begin{itemize}
...
This paper demonstrates that accurate day-ahead electricity price forecasting for ERCOT is achievable using gradient boosting with readily available weather data. The LightGBM model incorporating lag features, calendar variables, and weather covariates achieves an average MAE of \$9.35/MWh over the 2025 test period, with performance improving significantly during normal operating conditions. Quantile regression extends the framework to provide probabilistic forecasts with prediction intervals.
