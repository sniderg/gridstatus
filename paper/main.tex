...
\small \texttt{graydon.snider@email.com}}
\date{\today}
...
To capture local trends and volatility, we compute:
\begin{align}
    \bar{y}_{t,24} &= \frac{1}{24}\sum_{i=1}^{24} y_{t-24-i} \quad \text{(24-hour rolling mean)} \\
    \sigma_{t,24} &= \sqrt{\frac{1}{24}\sum_{i=1}^{24} (y_{t-24-i} - \bar{y}_{t,24})^2} \quad \text{(24-hour rolling std)}
\end{align}
...
\begin{itemize}
    ...
    \item Maximum depth: default (no limit)
    \item Random seed: 42
\end{itemize}

The full feature vector at time $t$ is:
\begin{equation}
    \mathbf{x}_t = [\mathbf{x}_t^{\text{lag}}, \mathbf{x}_t^{\text{rolling}}, \mathbf{x}_t^{\text{calendar}}, \mathbf{x}_t^{\text{weather}}]^\top
\end{equation}
...
Figure~\ref{fig:monthly} shows significant variation in forecast accuracy across months. Performance is strongest during mild shoulder seasons (September, October) and degrades during extreme weather periods.
...
Error varies systematically by hour of day (Figure~\ref{fig:hourly_error}). The highest errors occur during:
\begin{itemize}
...
The feature importance analysis confirms that temperature is the second-most important predictor after the 24-hour lag. Weather features collectively account for approximately 25\% of the total feature importance, justifying the integration of external weather data despite the additional complexity.
...
\begin{itemize}
    \item Quantile regression to provide prediction intervals
